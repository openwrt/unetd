...
static int cmd_pubkey(int argc, char **argv)
{
	...
	if (seckey_pq_len) {
		...
		if (pubkey_pq_len != MLD_44_PUBLICKEYBYTES) {
			INFO("Invalid post-quantum signing key\n");
			return 1;
		}
		pq_len = MLD_44_PUBLICKEYBYTES;
	}
...
static int cmd_generate_host(int argc, char **argv)
{
	...
	if (pq_keys) {
		if (seckey_pq_len && seckey_pq_len != SNTRUP761_SEC_SIZE) {
			INFO("Seeded post-quantum host keys are not supported\n");
			return 1;
		}
		if (!seckey_pq_len) {
...
static int cmd_generate(int argc, char **argv)
{
	...
	if (pq_keys) {
		if (seckey_pq_len && seckey_pq_len != MLD_44_SECRETKEYBYTES) {
			INFO("Invalid post-quantum signing key\n");
			return 1;
		}
		if (!seckey_pq_len) {
