		    ...
		    struct vlist_node *node_old)
{
	...
	if (h_new && h_old) {
		...
		if (h_old->kex_ctx.role != PEX_PQC_ROLE_NONE &&
		    h_new->kex_ctx.role != PEX_PQC_ROLE_NONE) {
...
network_hosts_connect_cb(struct uloop_timeout *t)
{
	...
	/* keep the timer alive on early exit, it resets the query rate limits above */
	if (!net->net_config.local_host)
		goto rearm;

	vlist_for_each_element(&net->peers, peer, node) {
		if (peer->kex_ctx.role == PEX_PQC_ROLE_NONE || peer->indirect)
			continue;
