...
#define PEX_PQC_CHALLENGE_LEN	32
...
struct pex_pqc_ctx {
	...
	/* responder: pending challenge issued on a rejected M1A, bound to that
	 * M1A's nonce so each handshake attempt gets a fresh one; initiator: the
	 * challenge received in a reject, mixed into the retransmitted M1A MAC */
	uint8_t challenge[PEX_PQC_CHALLENGE_LEN];
	uint8_t challenge_ref[CHACHA20_NONCE_SIZE];
	uint64_t last_reject_time;
...
struct pex_pqc_m1_reject {
	uint8_t m1a_ref[CHACHA20_NONCE_SIZE];
	uint8_t challenge[PEX_PQC_CHALLENGE_LEN];
	uint8_t mac[PEX_PQC_MAC_LEN];
};
